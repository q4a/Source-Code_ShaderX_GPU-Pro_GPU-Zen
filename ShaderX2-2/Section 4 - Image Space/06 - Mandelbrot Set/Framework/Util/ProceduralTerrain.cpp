...
void ProceduralTerrain::setBlock(Block *block){
	...
	if (GL_ATI_vertex_array_object){
		block->vaoBuffer = glNewObjectBufferATI(blockDensity * blockDensity * sizeof(TerrVertex), block->vertices, GL_STATIC_ATI);
...
void ProceduralTerrain::freeBlock(const Block *block){
	...
	if (GL_ATI_vertex_array_object){
		glDeleteObjectBufferATI(block->vaoBuffer);
	} else {
		//delete block.vertices;
	}
}
...
ProceduralTerrain::ProceduralTerrain(ProceduralTerrainFunc terrFunc, float bSize, int bDensity, int blockTexRepeat, int mBlocks){
	...
	int level = 0;
	do {
		indices.add(generateIndexArray(blockDensity, level));
		level++;
...
/*	bool res = blocks.goToFirst();
	while (res){
		freeBlock(blocks.getCurrent());
	...
	for (int z = minz; z <= maxz; z++){
		for (int x = minx; x <= maxx; x++){
			...
			if (d < viewDistance){
				...
				if (GL_ATI_vertex_array_object){
					glArrayObjectATI(GL_VERTEX_ARRAY,        3, GL_FLOAT, sizeof(TerrVertex), block->vaoBuffer, 0);
					glArrayObjectATI(GL_NORMAL_ARRAY,        3, GL_FLOAT, sizeof(TerrVertex), block->vaoBuffer, sizeof(Vertex));
					glArrayObjectATI(GL_TEXTURE_COORD_ARRAY, 2, GL_FLOAT, sizeof(TerrVertex), block->vaoBuffer, sizeof(Vertex) * 2);
				} else {
