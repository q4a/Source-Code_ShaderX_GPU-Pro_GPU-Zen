...
bool Direct3DRenderTarget::create(const int w, const int h, const bool mipmapped, const bool cubemap, RenderPixelFormat rPixelFormat){
	...
	if (isCubeMap = cubemap){
		dev->CreateCubeTexture(w, mipmapped? 0 : 1, D3DUSAGE_RENDERTARGET, format, D3DPOOL_DEFAULT, (LPDIRECT3DCUBETEXTURE9 *) &texID, NULL);
		for (unsigned int i = 0; i < 6; i++){
			((LPDIRECT3DCUBETEXTURE9) texID)->GetCubeMapSurface((D3DCUBEMAP_FACES) i, 0, &colorSurface[i]);
		}
	} else {
		dev->CreateTexture(w, h, mipmapped? 0 : 1, D3DUSAGE_RENDERTARGET, format, D3DPOOL_DEFAULT, (LPDIRECT3DTEXTURE9 *) &texID, NULL);
