...
protected:
	Plane planes[6];
public:
    void loadFrustum(const Matrix &modelView, const Matrix &projection);

    bool pointInFrustum(const Vertex &v) const;
	bool objectInFrustum(const Vertex *vertices, const int nVertices) const;
    bool sphereInFrustum(const Vertex &v, const float radius) const;
