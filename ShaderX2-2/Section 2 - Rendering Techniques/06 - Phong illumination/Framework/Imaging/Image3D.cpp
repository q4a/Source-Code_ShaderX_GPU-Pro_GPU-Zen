...
bool Image3D::loadFromFile(const char *fileName){
...
/*
	...
	file = fopen("Turb128.dds", "wb");
	header.width  = getWidth(0);
	header.height = getHeight(0);
	header.depth  = getDepth(0);
	fwrite(&header, sizeof(header), 1, file);
	size = getMipMappedSize(0, 1);

	unsigned char *src = getImage(0);
