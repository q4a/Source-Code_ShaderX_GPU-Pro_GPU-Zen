...
bool OpenGLRenderer::uploadTexture(Texture &texture){
	...
	if (texture.textureKind == TEXKIND_3D){
		...
		for (int i = 0; i < nImages; i++){
			...
			GLenum imgTarget = (texture.textureKind == TEXKIND_2D)? GL_TEXTURE_2D : GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB + i;
			...
			if (isPlainFormat(format)){
				...
				int mipMapLevel = 0;
				while ((src = img->getImage(mipMapLevel)) != NULL){
					glTexImage2D(imgTarget, mipMapLevel, internalFormat, img->getWidth(mipMapLevel), img->getHeight(mipMapLevel), 0, type[nChannels], GL_UNSIGNED_BYTE, src);
					mipMapLevel++;
				}
			} else {
