...
// Desc: 
//-----------------------------------------------------------------------------
bool CD3DApplication::ChooseInitialD3DSettings()
{
...
HRESULT CD3DApplication::Initialize3DEnvironment()
{
	...
	std::cout << "device of the " << pAdapterInfo->AdapterIdentifier.Description << " adapter (" << pAdapterInfo->AdapterIdentifier.Driver << ")." << std::endl<< std::endl;
...
INT CD3DApplication::Run()
{
    ...
    while( WM_QUIT != msg.message  )
    {
        ...
        if( bGotMsg )
        {
            ...
            if( hAccel == NULL || 0 == TranslateAccelerator( m_hWnd, hAccel, &msg ) )
            {
...
void CD3DApplication::UpdateStats()
{
    ...
    if( fTime - fLastTime > 1.0f )
    {
        ...
        TCHAR strFmt[100];
        D3DDISPLAYMODE mode;
        m_pD3D->GetAdapterDisplayMode(m_d3dSettings.AdapterOrdinal(), &mode);
        if( mode.Format == m_d3dsdBackBuffer.Format )
        {
            lstrcpyn( strFmt, D3DUtil_D3DFormatToString( mode.Format, false ), 100 );
        }
        else
        {
            _sntprintf( strFmt, 100, TEXT("backbuf %s, adapter %s"), 
                D3DUtil_D3DFormatToString( m_d3dsdBackBuffer.Format, false ), 
                D3DUtil_D3DFormatToString( mode.Format, false ) );
        }
        strFmt[99] = TEXT('\0');
        ...
        {
        case D3DMULTISAMPLE_NONE:  pstrMultiSample = TEXT(" (No Multisample)"); break;
