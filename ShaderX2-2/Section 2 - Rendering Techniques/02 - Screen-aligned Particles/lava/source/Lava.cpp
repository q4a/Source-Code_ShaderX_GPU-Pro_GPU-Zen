...
//The vertex shader - this one is different from the other example
//in that it has a growth factor on the particles
...
"vs_1_1 \n \
 ...
 dcl_texcoord3 v4 ; birth and death scale \n \
 dcl_color0 v5 ;birth color \n \
 dcl_color1 v6 ;death color \n \
 \
 ; size scale \n \
 mul r4.x, v4.x, TIME.y \n \
 mul r4.y, v4.y, TIME.x \n \
 add r4.x, r4.x, r4.y \n \
 \
 ; right \n \
...
//simple shader, just load a texture
...
HRESULT CMyD3DApplication::InitDeviceObjects()
{
	...
	m_pJunk = new CD3DMesh();

    if( FAILED( m_pSkyBox->Create( m_pd3dDevice, _T("skybox.x") ) ) )
        return D3DAPPERR_MEDIANOTFOUND;

	if( FAILED( m_pJunk->Create( m_pd3dDevice, _T("emitter.x") ) ) )
        return D3DAPPERR_MEDIANOTFOUND;	
...
HRESULT CMyD3DApplication::RestoreDeviceObjects()
{
    ...
    D3DXVECTOR3 vFromPt   = D3DXVECTOR3( 0.0f, 5.0f, -5.0f );
	...
	m_pJunk->RestoreDeviceObjects( m_pd3dDevice );

	//recreate the shaders and texture
	createVertexData();
	createShaders();

	D3DXCreateTextureFromFile( m_pd3dDevice, "lava.tga", &m_pTexture );
...
HRESULT CMyD3DApplication::Render()
{
    ...
    if( SUCCEEDED( m_pd3dDevice->BeginScene() ) )
    {
		...
		particleTime += m_DeltaTime * 0.25f;
		...
		if(particleTime > 1.0f)
		{
			...
			sndPlaySound("erupt.wav", SND_ASYNC);
		}
		
		timeData[0] = particleTime; 
		timeData[1] = 1.0f - particleTime; 
		timeData[2] = (particleTime * particleTime) * (0.5f) * -550.81f;
		timeData[3] = 0.0f; //(0.5f) * -9.81f;
...
HRESULT CMyD3DApplication::InvalidateDeviceObjects()
{
	...
	m_pJunk->InvalidateDeviceObjects(); 
...
HRESULT CMyD3DApplication::createVertexData()
{
	...
	for(int i = 0; i < NUM_PARTICLES; i++)
	{
		...
		color = D3DCOLOR_ARGB(255,255,11,11);

		//set up particles in vertex buffer

		setupParticle(vertices, i, center, 0.1f, 40.0f, color);
	}	
...
void CMyD3DApplication::setupParticle(ParticleVertex* vertexData, int index, D3DVECTOR center, float birthSize, float deathSize,
								 unsigned int color)
{
	...
	//create a random spherical velocity along the x and z axes
	...
	vertexOffset[0].rightFactor = -0.5f;
	vertexOffset[0].upFactor = 0.5f;	

	vertexOffset[0].velocity.x = velocity.x;
	vertexOffset[0].velocity.y = velocity.y;
	vertexOffset[0].velocity.z = velocity.z;
	vertexOffset[0].birthScale = birthSize;
	vertexOffset[0].deathScale = deathSize;
	...
	vertexOffset[1].rightFactor = 0.5f;
	vertexOffset[1].upFactor = 0.5f;

	vertexOffset[1].velocity.x = velocity.x;
	vertexOffset[1].velocity.y = velocity.y;
	vertexOffset[1].velocity.z = velocity.z;
	vertexOffset[1].birthScale = birthSize;
	vertexOffset[1].deathScale = deathSize;
	...
	vertexOffset[2].rightFactor = 0.5f;
	vertexOffset[2].upFactor = -0.5f;

	vertexOffset[2].velocity.x = velocity.x;
	vertexOffset[2].velocity.y = velocity.y;
	vertexOffset[2].velocity.z = velocity.z;
	vertexOffset[2].birthScale = birthSize;
	vertexOffset[2].deathScale = deathSize;
	...
	vertexOffset[3].rightFactor = -0.5f;
	vertexOffset[3].upFactor = -0.5f;	

	vertexOffset[3].velocity.x = velocity.x;
	vertexOffset[3].velocity.y = velocity.y;
	vertexOffset[3].velocity.z = velocity.z;
	vertexOffset[3].birthScale = birthSize;
	vertexOffset[3].deathScale = deathSize;
