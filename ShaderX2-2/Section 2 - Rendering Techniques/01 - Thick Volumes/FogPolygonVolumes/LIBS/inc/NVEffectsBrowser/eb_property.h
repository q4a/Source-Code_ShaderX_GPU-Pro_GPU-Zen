...
// hack to make map unsorted
struct ebcmp
{
    bool operator()(const EBString &s1, const EBString &s2) const
    {
        return true;
    }
};
...
typedef std::map<EBString, EBProperty*, ebcmp> t_mapProperties;
...
public:
	...
	virtual void WriteValue(IPropertySet* pObj, const EBValue& Value);
	virtual EBValue ReadValue(IPropertySet* pObj);
...
public:						
	// Property support
	virtual bool AddProperty(EBProperty* pProperty) = 0;
	virtual EBProperty* GetProperty(const EBString& PropertyKey) = 0;
	virtual void PropertyUpdateCallback(const EBProperty* pProperty, bool bWritten) = 0;
	virtual void UpdateProperties() =  0;
	virtual void DiscardProperties() = 0;
	virtual EBProperty* GetFirstProperty() = 0;
	virtual EBProperty* GetNextProperty() = 0;
	virtual void PropertyWritten(EBProperty* pProperty) = 0;
	virtual void PropertyRead(EBProperty* pProperty) = 0;
};


// Base class for all objects - enables property support
template <class T>
class EBPropertySet : public T
{
public:						
	EBPropertySet()
	{
	...
	virtual EBProperty* GetProperty(const EBString& PropertyKey)
	{
        ...
        t_mapProperties::const_iterator itrCurrent = m_mapProperties.begin();

        // need to manually search through map since its unsorted
        for (;itrCurrent != m_mapProperties.end(); itrCurrent++)
        {
            if (itrCurrent->first == PropertyKey)
                return itrCurrent->second;
        }
	...
	{
        // get last property because list is backwards 
		m_itrProperties = m_mapProperties.end();
        m_itrProperties--;

		if ((*m_itrProperties).second)
	...
	{
		if (m_itrProperties == m_mapProperties.begin())
			return NULL;
...
public:
	...
	virtual void Trigger(IPropertySet* pObj);
};
