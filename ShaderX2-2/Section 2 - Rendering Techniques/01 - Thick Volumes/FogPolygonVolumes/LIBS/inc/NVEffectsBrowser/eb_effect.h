...
#include <NVEffectsBrowser/eb_file.h>

#include <nv_renderdevice/invrenderdevice_d3d.h>
#include <nv_renderdevice/invrenderdevice_gl.h>
...
public:
	...
	virtual HRESULT ConfirmDevice(nv_renderdevice::INVRenderDevice* pRenderDevice)
	{
		...
		if (API() == GFXAPI_D3D || API() == GFXAPI_D3DCG)
		{
			nv_renderdevice::INVRenderDevice_D3D8* pRenderDeviceD3D8 = NULL;
			if (!pRenderDevice->GetInterface(IID_INVRenderDevice_D3D8, (void**)&pRenderDeviceD3D8))
			{
			...
			if (!pRenderDeviceD3D8->GetDevice(&device))
			{
				m_strLastError = "Failed to get D3D device interface";
				return E_FAIL;
			}
			...
			SAFE_RELEASE(pRenderDeviceD3D8);
			SAFE_RELEASE(device);
		}
		else
		{
			nv_renderdevice::INVRenderDevice_GL* pGLDevice = NULL;
			pRenderDevice->GetInterface(IID_INVRenderDevice_GL, (void**)&pGLDevice);
			if (pGLDevice)
			{
				PIXELFORMATDESCRIPTOR pfd;
				pGLDevice->GetPixelFormat(&pfd);

				hr = ConfirmDevice(&pfd);

				SAFE_RELEASE(pGLDevice);
			}
		}
	...
	virtual HRESULT Initialize(nv_renderdevice::INVRenderDevice* pRenderDevice)
	{
		...
		else
		{
			...
			{
				HGLRC hRC;
				pGLDevice->GetDeviceContext(&hRC);
	...
	virtual HRESULT ConfirmDevice(D3DCAPS8* caps, DWORD behavior, D3DFORMAT)
	{
		return E_FAIL;
	}
	virtual HRESULT Initialize(IDirect3DDevice8* pDevice)
	{
		return E_FAIL;
	}
