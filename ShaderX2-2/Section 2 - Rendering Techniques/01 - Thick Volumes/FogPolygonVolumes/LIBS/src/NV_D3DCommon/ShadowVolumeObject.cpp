...
ShadowVolumeObject::ShadowVolumeObject()
{
	::WingedEdgeObject();
...
HRESULT ShadowVolumeObject::Free()
{
...
void ShadowVolumeObject::PostInit()
{
	SimpleObject::PostInit();

	assert( m_wNumInd > 0 );
	assert( m_wNumVerts > 0 );

	m_wLastTriIndex = m_wNumInd;
	m_wOriginalModelTriIndices = m_wNumInd;
...
void	ShadowVolumeObject::GetTriCounts( UINT * pNumTotalTris, UINT * pNumWithoutShadowVolTris )
{
...
void ShadowVolumeObject::ExplodeFaces( float distance )
{
	// move faces out along vector from
	//  center of object
	
	m_wNumInd = m_wOriginalModelTriIndices;

	SimpleObject::ExplodeFaces( distance );

	m_wNumInd = m_wLastTriIndex;	
}


void	ShadowVolumeObject::DrawShadowVolume( NV_D3DDEVICETYPE*  pDev )
{
	assert( pDev != NULL );
	assert( m_wLastTriIndex > 0 );
	assert( m_wOriginalModelTriIndices > 0 );

	FDebug("ShadowVolumeObject::  Non-VertexBuffer rendering!  This will be slow!\n");

	pDev->DrawIndexedPrimitiveUP(	m_PrimType,
									0,					// min index
									m_wNumVerts,
									m_wLastTriIndex/3,	// prim count
									m_pIndices,
									D3DFMT_INDEX16,
									m_pVertices,
...
HRESULT		ShadowVolumeObject::BuildShadowVolume( float weld_edge_threshold )
{
	assert( m_bIsValid );
	...
	MakeFaceted();		// Create new vertices.  No new triangles
...
//	assert( IsWingedEdgeDataBuilt() );
	...
	V_INT * stitch_pairs = new V_INT[ m_wNumVerts ];
...
//gj					assert( false );
	...
	if( new_tris.size() > 0 )
	{
		int  new_num_ind = new_tris.size() + m_wNumInd;

		if( new_num_ind < 65536 )
		{
			m_wOriginalModelTriIndices = m_wNumInd;

			AllocateMoreIndices( (UINT) new_num_ind );
...
HRESULT		ShadowVolumeObject::BuildShadowVolume_CloseHoles( float weld_edge_threshold )
{
