...
HRESULT RenderToTextureBase::Initialize( NV_D3DDEVICETYPE * pDev,
														...
														bool use_stencil )
{
	...
	m_ppRTTTexture = new NV_LPDIRECT3DTEXTURE[ num_surfaces ]; 
	FAIL_IF_NULL( m_ppRTTTexture );

	m_ppRTTSurface = new NV_LPDIRECT3DSURFACE[ num_surfaces ];
    ...
    for( i = 0; i < num_surfaces; i++ )
    {
        ...
        hr = m_pD3DDev->CreateTexture( width, height, 1, 
									  ...
									  D3DFMT_A8R8G8B8, 
                                      D3DPOOL_DEFAULT, & m_ppRTTTexture[i]);
#endif
#ifdef NV_USING_D3D9
        hr = m_pD3DDev->CreateTexture( width, height, 1, 
                                      D3DUSAGE_RENDERTARGET,
...
#ifdef NV_USING_D3D8
	hr = m_pD3DDev->GetRenderTarget(	&m_pDefaultBackbufferColor );
#endif
#ifdef NV_USING_D3D9
...
HRESULT	RenderToTextureBase::SetDefaultBuffersToThese(	NV_IDIRECT3DSURFACE * pColor,
...
void	RenderToTextureBase::GetDefaultBackbuffers( NV_IDIRECT3DSURFACE ** ppColor,
														...
														bool set_depth )
{
	...
	if( m_pD3DDev != NULL )
	{
		NV_IDIRECT3DSURFACE * pColor = NULL;
		NV_IDIRECT3DSURFACE * pDepth = NULL;
...
#ifdef NV_USING_D3D8
		hr = m_pD3DDev->SetRenderTarget( pColor,
											m_pDefaultBackbufferDepth );
#endif
#ifdef NV_USING_D3D9
...
#ifdef NV_USING_D3D8
	hr = m_pD3DDev->SetRenderTarget( pColor, pDepth );
#endif
#ifdef NV_USING_D3D9
	hr = m_pD3DDev->SetRenderTarget( 0, pColor );
	ASSERT_AND_RET_IF_FAILED( hr );
