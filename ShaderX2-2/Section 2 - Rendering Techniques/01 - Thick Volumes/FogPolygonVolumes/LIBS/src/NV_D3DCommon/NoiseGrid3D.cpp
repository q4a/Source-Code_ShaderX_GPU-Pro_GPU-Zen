...
//@@@ noiseGrid3D simple implementation - not complete
// Generates noise values on 3D grid
// Does not create a volume of noise.  Instead it takes an
//   input 3D vector and generates noise for that point
//   based on the supplied frequencies & amplitudes
// Frequency determines spatial frequency of grid points, so it
//   determines how rapidly the noise varies with respect to
//   changes in the input x,y,z point.
								...
								const vector< GridNoiseComponent > * pvComponents )
{
	...
	for( nc = 0; nc < pvComponents->size(); nc ++ )
	{
		...
		//@@@@@@ always trilinear for now, regardless of noise component type
