...
HRESULT NVBScene::Load(const std::string& filename, LPDIRECT3DDEVICE9 device, GetFilePathCallback getFilePath)
{
	Free();
	m_Device = device;
	std::string fullname = (getFilePath ? (*getFilePath)(filename) : filename);
...
HRESULT NVBScene::LoadTexture(const std::string& name, LPDIRECT3DTEXTURE9& texture, GetFilePathCallback getFilePath, D3DFORMAT format, D3DPOOL pool)
{
	HRESULT hr;
	std::string fullname = (getFilePath ? (*getFilePath)(name) : name);
...
void NVBScene::Update(float time, const D3DXMATRIX* transform, unsigned int what)
{
	...
	unsigned int keyCamera;
	float subKeyCamera;
	if (timeCamera >= 0) {
		keyCamera = timeCamera;
