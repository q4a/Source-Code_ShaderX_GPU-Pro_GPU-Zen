...
#include <NVEffectsBrowser/singleton.h>
//#include "nvdebug.h"
#ifndef CHECK_D3DAPI
...
public:
	...
	virtual void GetVector(const D3DLOCKED_RECT& LockData, DWORD i, DWORD j, D3DXVECTOR3& outVector)
	{
		...
		outVector.x = static_cast<float>((dwData >> 16) & 0xFF);
		outVector.y = static_cast<float>((dwData >> 8) & 0xFF);
		outVector.z = static_cast<float>((dwData) & 0xFF);
		outVector /= 127.5f;
...
public:
	virtual void GetColors(const D3DLOCKED_RECT& LockData, DWORD i, DWORD j, float& fRed, float& fGreen, float& fBlue, float& fAlpha)
	{
		DWORD dwValue;
		GetData(LockData, i, j, dwValue);
		fBlue = static_cast<float>(dwValue & 0xFF)/ 255.0f;
		fGreen = static_cast<float>((dwValue >> 8) & 0xFF) / 255.0f;
		fRed = static_cast<float>((dwValue >> 16) & 0xFF) / 255.0f;
		fAlpha = 1.0f;
	}
	...
	{
		D3DXVECTOR3 vecScaled = (inVector + D3DXVECTOR3(1.0f, 1.0f, 1.0f)) * 127.5f;
		BYTE red   = static_cast<BYTE>(vecScaled.x);
		BYTE green = static_cast<BYTE>(vecScaled.y);
		BYTE blue  = static_cast<BYTE>(vecScaled.z);
		BYTE alpha = 0xFF;
		DWORD dwData = (DWORD)( ( (DWORD)alpha << 24 ) | ( (DWORD)red << 16 ) | ( (DWORD)green << 8 ) | ( (DWORD)blue << 0) );
		SetData(LockData, i, j, dwData);
	}
...
class NVDataFormat_Q8W8V8U8 : public NVDataFormat_32Bit
{
	virtual void SetVector(const D3DLOCKED_RECT& LockData, DWORD i, DWORD j, const D3DXVECTOR3& inVector)
	{
		...
		signed char red   = static_cast<signed char>(vecScaled.x);
		signed char green = static_cast<signed char>(vecScaled.y);
		signed char blue  = static_cast<signed char>(vecScaled.z);
		signed char alpha = 0;
...
public:
	...
	static LPDIRECT3DCUBETEXTURE9 CreateNormalizationCubeMap(LPDIRECT3DDEVICE9 pD3DDev, DWORD dwWidth, WORD dwMipmaps = 0, D3DPOOL Pool = D3DPOOL_MANAGED)
	{
		...
		if (dwMipmaps == 0)
			dwMipmaps = static_cast<WORD>(pCubeTexture->GetLevelCount());
		...
		for (DWORD dwLevel = 0; dwLevel < dwMipmaps; dwLevel++)
		{
			for (int i = 0; i < 6; i++)
			{
				...
				pCubeTexture->UnlockRect((D3DCUBEMAP_FACES)i, dwLevel);
			}
		}
