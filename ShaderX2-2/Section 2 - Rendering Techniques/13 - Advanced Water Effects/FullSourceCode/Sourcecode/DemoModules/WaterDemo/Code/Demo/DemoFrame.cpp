																	...
																	IGNU32						a_u32APIType )
{
	...
	//
	// Create camera, skydome, terrain and water
	//
	...
	m_pWater	= new WATER( m_pGfxContextInterface, m_pAssistant, m_pMixer );
}
...
void DEMOFRAME::Finit											(	void )
{
	...
	//
	// Delete camera, skydome, terrain and water
	//
	if( NULL!=m_pCamera )
		delete m_pCamera;
	if( NULL!=m_pSkydome )
...
IGNBOOL DEMOFRAME::ExecuteKeyboard								(	void )
{
	...
	{		
		...
		// F3
		//
		if( (true==Data.m_bKeyDown[IGN_ENUM_INPKEY_F3])&&(false==m_LastKeyboardData.m_bKeyDown[IGN_ENUM_INPKEY_F3]) )
		{
			if( false==m_bWaterWireframeModus )
				m_bWaterWireframeModus = true;
			else
				m_bWaterWireframeModus = false;
		}
		...
		// F5 - F8 
		//
//		if( (true==Data.m_bKeyDown[IGN_ENUM_INPKEY_F5])&&(false==m_LastKeyboardData.m_bKeyDown[IGN_ENUM_INPKEY_F5]) )
//		{
//			// ...
//		}
//		if( (true==Data.m_bKeyDown[IGN_ENUM_INPKEY_F6])&&(false==m_LastKeyboardData.m_bKeyDown[IGN_ENUM_INPKEY_F6]) )
//		{
//			// ...
//		}
//		if( (true==Data.m_bKeyDown[IGN_ENUM_INPKEY_F7])&&(false==m_LastKeyboardData.m_bKeyDown[IGN_ENUM_INPKEY_F7]) )
//		{
//			// ...
//		}
//		if( (true==Data.m_bKeyDown[IGN_ENUM_INPKEY_F8])&&(false==m_LastKeyboardData.m_bKeyDown[IGN_ENUM_INPKEY_F8]) )
//		{
//			// ...
//		}
		...
		// Space
		//
		if( (true==Data.m_bKeyDown[IGN_ENUM_INPKEY_SPACE])&&(false==m_LastKeyboardData.m_bKeyDown[IGN_ENUM_INPKEY_SPACE]) )
		{
/*			switch( m_TargetClass )
			{
			case	ENUM_TARGETCLASS_DX9:
					m_TargetClass = ENUM_TARGETCLASS_DX8;
					break;
			case	ENUM_TARGETCLASS_DX8:
					m_TargetClass = ENUM_TARGETCLASS_DX8GF3;
					break;
			case	ENUM_TARGETCLASS_DX8GF3:
					m_TargetClass = ENUM_TARGETCLASS_DX7;
					break;
...
*/
//			if( NULL!=m_pAssistant )
//				m_pAssistant->SaveImageToFile( NULL, "IGNscreenshot.bmp" );
		}

		//
		// Numpad -
		//
		if( true==Data.m_bKeyDown[IGN_ENUM_INPKEY_SUBTRACT] )
		{
			if( true==Data.m_bKeyDown[IGN_ENUM_INPKEY_RCONTROL] )
			{
				m_fWaterHeight -= 0.05f;
			}
			else
			{
				m_fWaterHeight -= 0.01f;
			}
		}
		...
		// Numpad +
		//
		if( true==Data.m_bKeyDown[IGN_ENUM_INPKEY_ADD] )
		{
			if( true==Data.m_bKeyDown[IGN_ENUM_INPKEY_RCONTROL] )
			{
				m_fWaterHeight += 0.05f;
...
void DEMOFRAME::ShowInfo										(	void )
{
	...
	if( ENUM_INFODISPLAY_SHOWALL==m_ShowInfo )
	{
		...
		{
		case	ENUM_TARGETCLASS_DX9:
				strcat( string, "\nTarget Hardware: DX9-Class" );
				break;
		case	ENUM_TARGETCLASS_DX8:
				strcat( string, "\nTarget Hardware: DX8-Class" );
				break;
		case	ENUM_TARGETCLASS_DX8GF3:
				strcat( string, "\nTarget Hardware: DX8-Class (with GeForce3 fix)" );
				break;
		case	ENUM_TARGETCLASS_DX7:
				strcat( string, "\nTarget Hardware: DX7-Class" );
				break;
		}
		strcat( string, "\n" );
		strcat( string, "\nKeys:" );
		...
		strcat( string, "\n F3          ->  Water Rendermode (Solid/Wireframe)" );
...
IGNBOOL DEMOFRAME::ProceedNextFrame								(	HWND						a_hWnd )
{
	...
	if( NULL!=m_pMixer )
	{
		...
		if( (0<Rect.right)&&(0<Rect.bottom) )
		{
			...
			if( NULL!=m_pSkydome )
			{
				m_pSkydome->AdvanceTime( 0.0000001f * 100.0f/static_cast<IGNFLOAT>( m_u32CurrentFPS ) );
				m_pSkydome->Proceed( 
					m_pCamera, 
			...
			//
			// Proceed with the water effect
			//
			if( NULL!=m_pWater )
				m_pWater->Proceed( 
					m_bWaterWireframeModus,
					m_TargetClass );
			else 
				return false;
