...
void TERRAIN::Init												(	IGN_OBJIF_GfxContext*	a_pGfxContextInterface )
{
	...
	{		
		...
		if( NULL!=m_pVertexData )
		{
			...
			VertexBufferDescription.m_uiLength	= 128*128*32;
			...
			structVertexFormatTerrain Terrain[128][128];
			IGNINT iRowPitch;
			IGNINT iSlicePitch;
			IGNU32* pBits;
			...
			if( NULL!=m_pHeightMap )
			{
				...
				for( IGNU32 u32ZIndex = 0; u32ZIndex<128; u32ZIndex++ )
				{
					...
					{
						Terrain[u32XIndex][u32ZIndex].m_Position.m_fX	=	-128.0f + 2*u32XIndex;
						Terrain[u32XIndex][u32ZIndex].m_Position.m_fY	=	-5.0f + (0x000000ff&(*(pBits+u32ZIndex*(iRowPitch/4)+u32XIndex)))/10.0f;
						Terrain[u32XIndex][u32ZIndex].m_Position.m_fZ	=	-128.0f + 2*u32ZIndex;
						Terrain[u32XIndex][u32ZIndex].m_TexCoord0_U		=	u32XIndex/4.0f;
						Terrain[u32XIndex][u32ZIndex].m_TexCoord0_V		=	u32ZIndex/4.0f;
					}
				}
				...
				//
				for( u32ZIndex = 0; u32ZIndex<128; u32ZIndex++ )
				{
					for( IGNU32 u32XIndex = 0; u32XIndex<128; u32XIndex++ )
					{
						if( 127==u32XIndex )
				...
				// Smooth all normals
				//
				IGN_STRUCT_3DVector SmoothedAreaNormal[128][128];
				for( u32ZIndex = 2; u32ZIndex<126; u32ZIndex++ )
				{
					for( IGNU32 u32XIndex = 2; u32XIndex<126; u32XIndex++ )
					{
