//-----------------------------------------------------------------------------
// File: TurbulentSun.cpp
//
// Desc: DirectX MFC dialog application created by the DirectX AppWizard
//-----------------------------------------------------------------------------
#define STRICT
#include "stdafx.h"
#include "TurbulentSun.h"
#include"resource.h"
//-----------------------------------------------------------------------------
// Application globals
//-----------------------------------------------------------------------------
TCHAR*          g_strAppTitle       = _T( "TurbulentSun" );
CApp            g_App;
...
int	i3=0;
void CAppForm::GenerateSphere(int numbofpoints)
{
	...
	m_pd3dDevice->CreateVertexBuffer( 8400*sizeof(CUSTOMVERTEX),
                                                  0, 0,
                                                  D3DPOOL_DEFAULT, &SunVB, NULL ) ;

	SunVB->Lock( 0, 0, (void**)&sphere, 0 );
         ...
         :CFormView( IDD_FORMVIEW )
{
    ...
    m_strWindowTitle            = TEXT( "TurbulentSun" );
	...
	for(int i=0;i<2000;i++)
		distort[i]=(float) (rand() / (float) RAND_MAX);
...
HRESULT CAppForm::InitDeviceObjects()
{
	...
	const char* vertOptions[] = { cgD3D9GetOptimalOptions(CG_PROFILE_VS_1_1), NULL };

	SunVertex=cgCreateProgramFromFile(Context,CG_SOURCE,"TurbSunVertex.cg",CG_PROFILE_VS_1_1,"main",vertOptions);
	Flare=cgCreateProgramFromFile(Context,CG_SOURCE,"Flare.cg",CG_PROFILE_VS_1_1,"main",vertOptions);
   ...
   const D3DVERTEXELEMENT9 decl[] =
	...
	assert(cgD3D9ValidateVertexDeclaration(Flare, decl));
	m_pd3dDevice->CreateVertexDeclaration(decl, &m_pDeclaration);

	SunPixel=cgCreateProgramFromFile(Context,CG_SOURCE,"TurbSunPixel.cg",CG_PROFILE_PS_2_X,"main",0);
	Blank=cgCreateProgramFromFile(Context,CG_SOURCE,"Blank.cg",CG_PROFILE_PS_1_1,"main",0);
	Text=cgCreateProgramFromFile(Context,CG_SOURCE,"Text.cg",CG_PROFILE_PS_1_1,"main",0);

	cgD3D9LoadProgram(SunVertex, true, 0);
	cgD3D9LoadProgram(SunPixel, true, 0);
	cgD3D9LoadProgram(Flare, true, 0);
	cgD3D9LoadProgram(Text, true, 0);
	cgD3D9LoadProgram(Blank, true, 0);

	WorldViewProj= cgGetNamedParameter(SunVertex, "ModelViewProj");
	Texture = cgGetNamedParameter(SunPixel, "BaseTexture");
	Time=cgGetNamedParameter(SunPixel, "Time");
	AmountOfVer=cgGetNamedParameter(Flare, "AmountOfVertexes");
	FlareWorldViewProj=cgGetNamedParameter(Flare, "ModelViewProj");
...
HRESULT CAppForm::RestoreDeviceObjects()
{
	...
	m_pd3dDevice->CreateVertexBuffer( 2*(steps+1)*sizeof(CUSTOMVERTEX),
                                                  D3DUSAGE_DYNAMIC, 0,
    											  D3DPOOL_DEFAULT, &SunFlare, NULL ) ;
	


	D3DXCreateTexture(m_pd3dDevice,512,1,1,0,D3DFMT_A16B16G16R16F,D3DPOOL_DEFAULT ,&pTexture);
    ...
    m_pd3dDevice->SetRenderState( D3DRS_AMBIENT,        FALSE );
    ...
    D3DXMATRIX matView;
    D3DXVECTOR3 vFromPt   = D3DXVECTOR3( 3.0f, 3.0f, 0.0f );
    ...
    m_pd3dDevice->SetRenderState( D3DRS_LIGHTING, FALSE );
	    ...
	    //D3DXMATRIX worldViewProj = matIdentity * matView * matProj;
        //D3DXMatrixTranspose(&worldViewProj, &worldViewProj);
      //  cgD3D9SetUniformMatrix(WorldViewProj, &worldViewProj);
...
HRESULT CAppForm::Render()
{
	...
	D3DXMatrixRotationX( &matRotX, 0.7853981633974483 );
    D3DXMatrixRotationY( &matRotY, 1.5707963267948965 );
    D3DXMatrixMultiply( &m_World, &matRotX, &matRotY ); 
	mv = m_World*m_View * m_Projection;
	cgD3D9SetUniformMatrix(FlareWorldViewProj, &mv);
	cgD3D9SetUniform(Time,&time);
	cgD3D9SetUniform(AmountOfVer, &steps);
    ...
    if( SUCCEEDED( m_pd3dDevice->BeginScene() ) )
    {
		...
		RenderText();
		cgD3D9BindProgram(SunVertex);
		cgD3D9BindProgram(SunPixel);
		m_pd3dDevice->SetStreamSource( 0,SunVB ,0, sizeof(CUSTOMVERTEX) );
		m_pd3dDevice->SetVertexDeclaration(m_pDeclaration);
		m_pd3dDevice->DrawPrimitive( D3DPT_TRIANGLESTRIP, 0, i3-2 );
		

		cgD3D9BindProgram(Blank);
		cgD3D9BindProgram(Flare);
	
     	CUSTOMVERTEX *fnull;
		SunFlare->Lock( 0, 0, (void**)&fnull, 0 );
		int index=0;
        ...
        m_pd3dDevice->SetStreamSource( 0,SunFlare ,0, sizeof(CUSTOMVERTEX) );
		m_pd3dDevice->DrawPrimitive( D3DPT_TRIANGLESTRIP, 0, (steps+1)*2 );
