...
void
pgTerrainPVS::calc(const unsigned char* nMinData,
			...
			for(srcX=0; srcX<dataX; srcX++)
				...
				for(dstY=0; dstY<dataY; dstY++)
					for(dstX=0; dstX<dataX; dstX++)
					{
						...
						float	p, step = 1.0f / (float)sqrt((dstX-srcX)*(dstX-srcX) + (dstY-srcY)*(dstY-srcY)),
								dx = (float)dstX-srcX,
								dy = (float)dstY-srcY,
								dz = (float)hMax-srcZ;
...
pgTerrainPVS::shrink()
{
	pgTerrainPVS* oldPVS = new pgTerrainPVS();
	oldPVS->dataX = dataX;
	oldPVS->dataY = dataY;
	oldPVS->dataZ = dataZ;
...
bool
pgTerrainPVS::save(const char* nFileName)
{
	if(!data)
		return false;
...
bool
pgTerrainPVS::load(const char* nFileName)
{
...
void
pgTerrainPVS::setVisible(int nX0, int nY0, int nZ0, int nX1, int nY1, bool nValue)
{
...
bool
pgTerrainPVS::isVisible(int nX0, int nY0, int nZ0, int nX1, int nY1) const
{
	//if(dataDZ==0)
	//	return true;
	//nZ0 /= dataDZ;
...
unsigned int
pgTerrainPVS::getIdx(int nX0, int nY0, int nZ0, int nX1, int nY1) const
{
