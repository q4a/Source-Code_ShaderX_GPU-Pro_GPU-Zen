...
// these cause the barycentric coordinates to be quantised, causes holes at the moment
...
HRESULT CMyD3DApplication::InitDeviceObjects()
{
	...
	// default to barycentric coordinate method
...
HRESULT CMyD3DApplication::GenerateBaryConstants()
{
	...
	// calculate areal (normalised barycentric) coordinates
	for( unsigned int index=0; index < m_numIndices/3;index++)
	{
		// 1st vertex : i=1, j=k=0
		// 2nd vertex : j=1, i=k=0
		// 3rd vertex : k=1, i=j=0 (k is not stored though)

		unsigned int packVert;
		unsigned int i,j,k;

		i = 255; j = k = 0; // 1st vertex
		packVert = (i << 16) + (j << 8) + (index << 0); // correct for D3DCOLOR unpacking
		outStream[ (index*3) + 0 ] = packVert;
		j = 255; i = k = 0; // 2nd vertex
		packVert = (i << 16) + (j << 8) + (index << 0); // correct for D3DCOLOR unpacking
		outStream[ (index*3) + 1 ] = packVert;
		k = 255; i = j = 0; // 3rd vertex
		packVert = (i << 16) + (j << 8) + (index << 0); // correct for D3DCOLOR unpacking
...
HRESULT CMyD3DApplication::GenerateD3DNPatchMesh( unsigned int LOD )
{
	...
	LOD = LOD + 1; // difference in D3D and my counting
	...
	// determine the size of the mesh needed to hold the tesselated mesh
...
HRESULT CMyD3DApplication::RenderText()
{
	...
	} else if( m_displayMethod == DM_NPATCH )
	{
		...
		fNextLine += 20.0f;
	    sprintf( szMsg, "Lod Level %d", m_lod );
		m_pFont->DrawText( 2, fNextLine, fontColor, szMsg );
	} else if( m_displayMethod == DM_LINEAR )
	{
...
void CMyD3DApplication::CleanDisplayMethod()
{
	...
	delete m_baryPos; m_baryPos = 0;

	delete m_npatchPos; m_npatchPos = 0;
	delete m_npatchNorm; m_npatchNorm = 0;
