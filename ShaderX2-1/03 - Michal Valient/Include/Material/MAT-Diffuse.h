...
class CMaterialDiffuseContext : public CDX8MaterialContext {
protected:
...
public:
    ...
    CMaterialDiffuseContext();
    virtual ~CMaterialDiffuseContext();
    virtual int QueryProperty(const char *PropertyName);
    virtual bool SetProperty(int PropertyID, const void *Property);
};

class CMaterialDiffuse : public CDX8Material {
protected:
    CMaterialDiffuseContext *CurrentContext;
...
public:
    ...
    ///Const, dest & more
    CMaterialDiffuse(char *ObjName,CEngine *OwnerEngine);

    MAKE_CLSCREATE(CMaterialDiffuse);
    MAKE_CLSNAME("CMaterialDiffuse");
    MAKE_DEFAULTGARBAGE()
};
