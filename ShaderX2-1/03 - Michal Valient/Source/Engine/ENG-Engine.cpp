...
CEngine::~CEngine() {
    ...
    if (ShaderManager!=NULL) delete ShaderManager;
    if (MaterialManager!=NULL) delete MaterialManager;
    if (TextureManager!=NULL) delete TextureManager;
    if (MeshManager!=NULL) delete MeshManager;
    if (FileManager!=NULL) delete FileManager;
    if (RenderSettings!=NULL) delete RenderSettings;
    if (Gfx!=NULL) delete Gfx;
    if (Window!=NULL) delete Window;
    if (EngineDefaults!=NULL) delete EngineDefaults;
    if (RuntimeCreator!=NULL) delete RuntimeCreator;
...
//        Window->CreateWnd("MainEngineWindow",WS_EX_TOPMOST,WS_OVERLAPPEDWINDOW,0,0,Width,Height);
    ...
    if (!Fullscreen) {
        ...
        presentation_params.BackBufferFormat = mode.Format;
