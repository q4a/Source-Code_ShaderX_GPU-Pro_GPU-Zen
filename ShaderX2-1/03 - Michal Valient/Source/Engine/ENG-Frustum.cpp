...
//This code is taken from GameTutorials
...
void CFrustum::CalculateFrustum(const D3DXMATRIX &mview, const D3DXMATRIX &projection) {    
	float   proj[16];								// This will hold our projection matrix
	float   modl[16];								// This will hold our modelview matrix
	float   clip[16];								// This will hold the clipping planes

	// PROJECTION_MATRIX to abstract our projection matrix.
	// It then stores the matrix into an array of [16].
    memcpy(proj,projection,sizeof(proj));

	// MODELVIEW_MATRIX, we can abstract our model view matrix.
	// This also stores it in an array of [16].
    memcpy(modl,mview,sizeof(modl));
...
bool CFrustum::SphereInFrustum(const D3DXVECTOR3 &center, const float radius)
{
...
bool CFrustum::CubeInFrustum( const D3DXVECTOR3 &center, const float size )
{
...
CFrustum::CFrustum(char *ObjName,CEngine *OwnerEngine) : CBaseClass(ObjName, OwnerEngine) {
}
