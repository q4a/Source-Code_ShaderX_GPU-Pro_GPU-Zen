...
CSimpleScene::~CSimpleScene(void) {
    ...
    //First of all unset material
    OwnerEngine->GetMaterialManager()->SetMaterial(NULL,NULL,0,*OwnerEngine->GetRenderSettings());
...
void CSimpleScene::Render(CRenderSettings &Settings) {
    ...
    if (Objects->GetItemCount()>0) {
        ...
        //Compute camera view
    //    this->CurrentCamera->RotateLR((float)Settings.RenderTime);
        //float qqq = (float)Settings.RenderID * 4.0f * 0.001f;
        //this->CurrentCamera->RotateLR(qqq);
    ...
    //    this->CurrentCamera->RotateLR((float)Settings.RenderTime);
        ...
        Settings.Direct3DDevice->Clear( 0, NULL, D3DCLEAR_TARGET | D3DCLEAR_STENCIL | D3DCLEAR_ZBUFFER, D3DCOLOR_XRGB(255,0,255), 1.0f, 0 );
        Settings.Direct3DDevice->BeginScene();
        ...
        Settings.Direct3DDevice->EndScene();
    }
    _LOGE(logE,"Rendered");
}
...
bool CSimpleScene::LoadXML(CXMLNode *Node, CRenderSettings &Settings) {
    ...
    CDX8Material *MPhong = (CDX8Material *)OwnerEngine->GetMaterialManager()->Query(D3_GRI_Mat_PixelPhong);
    if (MPhong == NULL) {
            ...
            if (strcmp(tmpNode->GetName().c_str(), "D3F_OBJECT") == 0) {
                ...
                if (!tmpIsTarget) {
                    ...
                    for (int i=0; i<Msh->GetSubsetCount(); i++) {
                        ...
                        CDX8MaterialContext *Context = (CDX8MaterialContext *)MPhong->CreateContextFromXML(tmpMatNode, Object, Settings);
