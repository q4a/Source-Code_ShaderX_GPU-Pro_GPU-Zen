...
// The shadow volume constructed is also capped at both ends. This is a 
// requirement of the depth fail stencil shadow algorithm.
//-----------------------------------------------------------------------------
HRESULT CShadow::BuildShadowVolume( LPD3DXMESH pMesh, D3DXVECTOR3 vLight )
{
  ...
  for( DWORD i=0; i<dwNumFaces; i++ )
  {
    ...
    if( D3DXVec3Dot( &vFaceNormal, &vLightDir ) >= 0.0f )
    {
      ...
      m_pVertices[m_dwNumOfVertices++] = v0;
      m_pVertices[m_dwNumOfVertices++] = v1;
      m_pVertices[m_dwNumOfVertices++] = v2;

      // Add all three edges for silhouette testing
  ...
  D3DXVECTOR3 vTriFanStart = pVertices[pEdges[(dwNumEdges-1)*2+1]].p;  // Use last vertex as origin for back capping: tri fan
  vExtrusionDir = vTriFanStart - vLight;
  D3DXVec3Normalize( &vExtrusionDir, &vExtrusionDir );
  vTriFanStart.x = vTriFanStart.x + vExtrusionDir.x * g_fExt;
  vTriFanStart.y = vTriFanStart.y + vExtrusionDir.y * g_fExt;
  vTriFanStart.z = vTriFanStart.z + vExtrusionDir.z * g_fExt;
  ...
  for( i=0; i<dwNumEdges; i++ )
  {
    ...
    // Extrude silhouette edges to form side wall of shadow volume
    D3DXVECTOR3 v3;
    D3DXVECTOR3 v4;
    ...
    // Add a quad (two triangles) to the vertex list, make sure the quad faces outwards!
    m_pVertices[m_dwNumOfVertices++] = v1;
    ...
    m_pVertices[m_dwNumOfVertices++] = vTriFanStart;
    m_pVertices[m_dwNumOfVertices++] = v4;
    m_pVertices[m_dwNumOfVertices++] = v3;
  }
