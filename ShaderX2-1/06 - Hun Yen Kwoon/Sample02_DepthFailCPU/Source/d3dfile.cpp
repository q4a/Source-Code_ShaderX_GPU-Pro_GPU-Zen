...
// Desc:
//-----------------------------------------------------------------------------
// HACK: Hack to force textures to load from designated dir
HRESULT CD3DMesh::Create( LPDIRECT3DDEVICE8 pd3dDevice, TCHAR* strFileDir, TCHAR* strFilename, BOOL compressTex )
{
  // HACK: Force textures to load from designated dir
  char t[256];
  sprintf(t, "%s%s", strFileDir, strFilename);  // Appends dir with .x filename
  strFilename = t;
    ...
    if( pMtrlBuffer && m_dwNumMaterials > 0 )
    {
        ...
        for( DWORD i=0; i<m_dwNumMaterials; i++ )
        {
            ...
            if( d3dxMtrls[i].pTextureFilename )
            {
                ...
                // HACK: Hack to force textures to load from designated dir
                sprintf(t, "%s%s", strFileDir, d3dxMtrls[i].pTextureFilename);
                DXUtil_ConvertAnsiStringToGeneric( strTextureTemp, t);

                DXUtil_FindMediaFile( strTexture, strTextureTemp );

                // HACK: Major hack here to do dxt1 compress specified by compressTex flag
                if (!compressTex)
                {
                  if ( FAILED(D3DXCreateTextureFromFile( pd3dDevice, strTexture, &m_pTextures[i]) ) )
                    m_pTextures[i] = NULL;
                }
                else  // Compress to dxt1
                {
                  ...
                  else
                  {
                    ...
                    if ( FAILED (D3DXCreateTexture(pd3dDevice, desc.Width, desc.Height, 0,
                    ...
                    {
                      for(DWORD j=0; j<(tempTex->GetLevelCount()); j++)
                      {
                        LPDIRECT3DSURFACE8 destSurface;
                        LPDIRECT3DSURFACE8 sourceSurface;
                        m_pTextures[i]->GetSurfaceLevel(j, &destSurface);
                        tempTex->GetSurfaceLevel(j, &sourceSurface);
                        D3DXLoadSurfaceFromSurface(destSurface, NULL, NULL, 
...
//  LPD3DXWELDEPSILONS pEpsilon = new _D3DXWELDEPSILONS;
//  pEpsilon->Flags = D3DXWELDEPSILONS_WELDALL;
//  pEpsilon->Normal = 0.0f;
//  pEpsilon->SkinWeights = 0.0f;
//  pEpsilon->Tex[0] = 0.0f;
//  pEpsilon->Tex[1] = 0.0f;
//  pEpsilon->Tex[2] = 0.0f;
//  pEpsilon->Tex[3] = 0.0f;
...
//  pEpsilon->Tex[7] = 0.0f;
//
//  DWORD dwNumFace = m_pSysMemMesh->GetNumFaces()
//    DWORD adjacency[dwNumFace*3];
//
//if (FAILED(D3DXWeldVertices(
//  m_pSysMemMesh,  // CONST LPD3DXMESH pMesh,
//  pEpsilon,       // LPD3DXWELDEPSILONS pEpsilon,
//  NULL,           // CONST DWORD* pAdjacencyIn,
//  &adjacency,      // DWORD* pAdjacencyOut,
//  DWORD* pFaceRemap,
