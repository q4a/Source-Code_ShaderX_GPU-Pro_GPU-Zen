...
//-----------------------------------------------------------------------------
// SetShaderConstants()
// Sets the shader constants for use within the vertex program
//-----------------------------------------------------------------------------
...
{
  D3DXMATRIX matClip, matInvWorld;
  D3DXMatrixMultiply( &matClip, matWorld, matView );  
  D3DXMatrixMultiply( &matClip, &matClip, matProj );
  D3DXMatrixInverse( &matInvWorld, NULL, matWorld );

  D3DXVECTOR4 vConst( 0.0f, 0.0f, 0.0f, m_fExtrusionLen );

  // Yellowish green hue for drawing shadow volume if needed
  D3DXVECTOR4 vColor( 0.3f, 0.4f, 0.2f, 0.0f );

  // Light pos in world space
  D3DXVECTOR4 objectLightPos = D3DXVECTOR4( pLight->Position.x, 
        pLight->Position.y, pLight->Position.z, 1.0f );
  
  // Transform light pos to object space
  D3DXVec4Transform( &objectLightPos, &objectLightPos, &matInvWorld );

  // Set the shader constants
  m_pd3dDevice->SetVertexShaderConstant( 0, &vConst, 1 );
  m_pd3dDevice->SetVertexShaderConstant( 1, &objectLightPos, 1 );
  m_pd3dDevice->SetVertexShaderConstant( 2, &matClip, 4 );
  m_pd3dDevice->SetVertexShaderConstant( 6, &vColor, 1 );
}


//-----------------------------------------------------------------------------
// SetExtrusionLength()
// Sets the extrusion length to be used as shader constant during rendering
//-----------------------------------------------------------------------------
void CShadow::SetExtrusionLength( float fLen )
{
  if ( fLen > 0.0f )
    m_fExtrusionLen = fLen;
}
