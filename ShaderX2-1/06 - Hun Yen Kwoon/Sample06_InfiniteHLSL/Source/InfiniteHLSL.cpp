...
//-----------------------------------------------------------------------------
// CInfiniteHLSL constructor
//-----------------------------------------------------------------------------
CInfiniteHLSL::CInfiniteHLSL()
{
  m_strWindowTitle                    = _T( "InfiniteHLSL Shadow Volume Sample" );
...
//-----------------------------------------------------------------------------
HRESULT CInfiniteHLSL::ConfirmDevice( D3DCAPS9* pCaps, DWORD dwBehavior,
...
// OneTimeSceneInit()
//-----------------------------------------------------------------------------
HRESULT CInfiniteHLSL::OneTimeSceneInit()
{
...
//-----------------------------------------------------------------------------
HRESULT CInfiniteHLSL::RestoreDeviceObjects()
{
...
//-----------------------------------------------------------------------------
HRESULT CInfiniteHLSL::InvalidateDeviceObjects()
{
...
// DeleteDeviceObjects()
//-----------------------------------------------------------------------------
HRESULT CInfiniteHLSL::DeleteDeviceObjects()
{
...
// RenderScene()
//-----------------------------------------------------------------------------
HRESULT CInfiniteHLSL::RenderScene()
{
  if ( m_bShowObjects )
  {
...
//-----------------------------------------------------------------------------
HRESULT CInfiniteHLSL::RenderShadowVolume()
{
  // Show shadow volume?
  if ( m_bShowShadowVol )
  {
  ...
  if( m_pEffect != NULL )
  {
    ...
    // We set up the 2 matrices and 1 vector needed by the HLSL code
    // All the derivation is similar to that explained in the InfiniteGPU
    // sample. For more information, please refer to the ShaderX2 book.
    D3DXMATRIX  mLightClip, mInvObj, mLight, mInvLight, mWorldInvLight;
    D3DXVECTOR4 currLightPos = D3DXVECTOR4( m_Light.Position.x, 
                                            m_Light.Position.y, 
                                            m_Light.Position.z, 
                                            1.0f );
    // Remember, we only factor in the translational values for the light.
    D3DXMatrixTranslation( &mLight, m_Light.Position.x,
                           m_Light.Position.y, m_Light.Position.z );
    D3DXMatrixTranslation( &mInvLight, -m_Light.Position.x,
                           -m_Light.Position.y, -m_Light.Position.z );
    mLightClip = mLight * m_matView * m_matProject;
    ...
    for( iPass = 0; iPass < nPasses; iPass ++ )
    {
      ...
      mWorldInvLight = m_matObject * mInvLight;
      ...
      m_pEffect->SetMatrix( m_hmLightClip, &mLightClip );
      m_pEffect->SetMatrix( m_hmWIL, &mWorldInvLight );
      m_pShadow->RenderShadow();

      // Shadow volume of object 2
      mWorldInvLight = m_matObject2 * mInvLight;
...
// to shade. 
//-----------------------------------------------------------------------------
HRESULT CInfiniteHLSL::DrawShadow()
{
...
// FrameMove()
//-----------------------------------------------------------------------------
HRESULT CInfiniteHLSL::FrameMove()
{
...
// FinalCleanup()
//-----------------------------------------------------------------------------
HRESULT CInfiniteHLSL::FinalCleanup()
{
...
//-----------------------------------------------------------------------------
HRESULT CInfiniteHLSL::LoadEffectsFile()
{
  HRESULT hr = S_OK;
  ...
  for( UINT iParam = 0; iParam < m_EffectDesc.Parameters; iParam++ )
  {
    ...
    if ( strcmpi( ParamDesc.Name, "mLightClip" ) == 0 )
      m_hmLightClip = hParam;
    else
    if ( strcmpi( ParamDesc.Name, "mWorldInvLight" ) == 0 )
      m_hmWIL = hParam;
  }
...
// MsgProc()
//-----------------------------------------------------------------------------
LRESULT CInfiniteHLSL::MsgProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
  if ( WM_KEYDOWN == uMsg )
  {
    m_bKey[wParam] = 1;
