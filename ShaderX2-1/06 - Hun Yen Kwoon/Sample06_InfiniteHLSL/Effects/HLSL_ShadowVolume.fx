...
float4x4  mLightClip;       // Light*View*Project
float4x4  mWorldInvLight;   // World*InvLight transformation matrix
  ...
  float4 Normal : NORMAL )
{
  ...
  float fMask = ( fDotResult < 0.0f ) ? 0.0f : 1.0f;
  
  // fMask = 0.0f for vertex to be extruded
  float4 vMask = { 1.0f, 1.0f, 1.0f, fMask };
  ...
  Out.Pos = mul( Pos * vMask, mLightClip );
