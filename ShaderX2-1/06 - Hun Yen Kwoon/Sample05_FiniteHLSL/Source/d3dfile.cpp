...
HRESULT CD3DMesh::Create(LPDIRECT3DDEVICE9 pd3dDevice, TCHAR* strFileDir, TCHAR* strFilename)
{
  ...
  strFilename = t;
    ...
    if( pMtrlBuffer && m_dwNumMaterials > 0 )
    {
        ...
        for( DWORD i=0; i<m_dwNumMaterials; i++ )
        {
            ...
            // HACK: Missing line in DX9.0
            m_pMaterials[i].Ambient = m_pMaterials[i].Diffuse;  // Bug delieved with DX9.0

            m_pTextures[i]          = NULL;

            // Create a texture
            // HACK: To handle no texture '_' exported by MilkShape3D
            if( (d3dxMtrls[i].pTextureFilename)&&(strcmp(d3dxMtrls[i].pTextureFilename, "_")) )
            {
                ...
                DXUtil_ConvertAnsiStringToGenericCb( strTextureTemp, t/*d3dxMtrls[i].pTextureFilename*/, sizeof(strTextureTemp) );
